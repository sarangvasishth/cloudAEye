...
export default function Index(props) {
  ...
  useEffect(() => {
    fetchRequests(page);
    if (page && search) {
      history.push(`/approved?page=${page}&search=${search}`);
    } else if (page && !search) {
      history.push(`/approved?page=${page}`);
    } else if (!page && search) {
      history.push(`/approved?page=1&search=${search}`);
    } else {
      history.push(`/approved?page=1`);
    }
  }, []);
  ...
  const fetchRequests = async (page) => {
    setLoader(true);
    try {
      const response = await getRequests(page, searchText, "APPROVED");
  ...
  const handlePageChange = (page) => {
    ...
    fetchRequests(page);
    if (searchText) {
      history.push(`/approved?page=${page}&search=${searchText}`);
    } else {
      history.push(`/approved?page=${page}`);
    }
  };
  ...
  const handleSearch = async () => {
    ...
    history.push(`/approved?page=${DEFAULT_PAGE}&search=${searchText}`);
  };
    ...
    <>
      <div className="row justify-content-center mx-0 px-0">
        ...
        {loader ? (
          ...
          <NoRecordsFound displayText="No approved requests found!" />
        )}
      </div>
    </>
  );
